...
const useGetWorkshopOrders = (
  ...
  const fetchWorkshopOrders = async () => {
    ...
    if (order_statuses) {
      order_statuses.forEach((o) => queryParams.append('order_statuses', o.toString()))
    }
    if (customer_ids) {
      customer_ids.forEach((c) => queryParams.append('customer_ids', c.toString()))
    }
    if (branches_ids) {
      branches_ids.forEach((b) => queryParams.append('branches_ids', b.toString()))
    }
    if (payment_statuses) {
      payment_statuses.forEach((p) => queryParams.append('payment_statuses', p.toString()))
    }
    if (workshop_ids) {
      workshop_ids.forEach((w) => queryParams.append('workshop_ids', w.toString()))
    }
    if (workshop_manager_ids) {
      workshop_manager_ids.forEach((p) => queryParams.append('workshop_manager_ids', p.toString()))
    }
    if (payment_types) queryParams.append('payment_types', payment_types.toString())
