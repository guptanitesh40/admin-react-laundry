...
const BranchForm: React.FC = () => {
  ...
  >
    <span>
      {companies.find(
        (company) => company.company_id === Number(formData.company_id)
      ...
      {companies.map((company) => (
        <li
          ...
          className={`cursor-pointer hover:bg-gray-100 ${
            company.company_id === Number(formData.company_id) ? "bg-gray-200" : ""
          }`}
          onClick={() => handleDropdownChange(company.company_id, company.company_name)}
        >
          <div className="block px-4 py-2 text-sm">{company.company_name}</div>
        </li>
      ))}
    </ul>
  )}
